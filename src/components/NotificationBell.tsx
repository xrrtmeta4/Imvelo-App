...
// Deleted alerts are removed from the database AND remembered locally, so an
// identical alert regenerated by the weather job never re-appears.
const dismissedKey = (userId: string) => `imvelo-dismissed-alerts-${userId}`;
const signature = (n: Pick<Notification, 'alert_type' | 'message'>) =>
  `${n.alert_type}::${n.message}`;

const readDismissed = (userId: string): string[] => {
  try {
...
const addDismissed = (userId: string, sigs: string[]) => {
  try {
    const next = Array.from(new Set([...readDismissed(userId), ...sigs])).slice(-200);
    window.localStorage.setItem(dismissedKey(userId), JSON.stringify(next));
  } catch {
    /* storage unavailable — database delete still applies */
  }
};
...
const NotificationBell = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    if (!error && data) {
      const dismissed = new Set(readDismissed(user.id));
      setNotifications(data.filter((n) => !dismissed.has(signature(n))));
    }
    setLoading(false);
  }, [user]);
  ...
  const deleteNotification = async (id: string) => {
    ...
    if (user && target) addDismissed(user.id, [signature(target)]);
  ...
  const deleteAllNotifications = async () => {
    if (!user) return;
    addDismissed(user.id, notifications.map(signature));
